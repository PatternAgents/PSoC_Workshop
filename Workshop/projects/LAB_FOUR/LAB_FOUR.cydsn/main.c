...
CY_ISR( ADC_1_IRQ_Handler )
{
    ...
    if((intr_status & ADC_1_EOS_MASK) != 0u)
    {
        ...
        if((range_status & (uint32)(1ul << CH0_N)) != 0u) 
        {
            ...
            result[CH1_N] = ADC_1_GetResult16(CH1_N);
...
int main()
{
    ...
    for(;;)
    {
        ...
        if((dataReady & ADC_1_EOS_MASK) != 0u)
        {
            ...
            res1 = ADC_1_CountsTo_mVolts(CH1_N, result[CH1_N]);
        }    
        ...
        if((dataReady & ADC_1_INJ_EOC_MASK) != 0u)
        {
            ...
            *******************************************************************************/
            ...
            /* Print voltage value of A0 to UART */
            sprintf(
                uartLine, "A0 measured voltage: %dmV \r\n",
                (uint16) res
                );
            
            UART_1_UartPutString(uartLine);
 
            /* Print voltage value of A1 to UART */
            sprintf(
                uartLine, "A1 measured voltage: %dmV \r\n",
                (uint16) res1
                );
            ...
            /* Print switch value to UART */
            sprintf(
                uartLine, "Switches: %d, %d, %d \r\n",
                SW1, SW2, SW3
                );
